...
class AppState extends ChangeNotifier with SupabaseProfileMixin {
  ...
  String _normalizeNotificationType(String? raw) {
    ...
    switch (value) {
      ...
      default:
        AppLogger.warning('[NOTIF] Unknown notification type: $raw', tag: _logTag);
        return 'system';
    }
  }
  ...
  String _notificationSemanticKey({
    ...
    final isDebate = normalized.startsWith('debate_');
    return [
      normalized,
      fromUserId ?? '',
      isDebate ? '' : (postId ?? ''),
      isDebate ? (discussionId ?? '') : '',
      commentId ?? '',
    ].join('|');
  }

  String _notificationSemanticKeyFromApp(AppNotification n) {
    final isDebate = n.type == NotificationType.debateComment ||
        n.type == NotificationType.debateReply ||
        n.type == NotificationType.debateLike ||
        n.type == NotificationType.debateVote;

    return [
      notificationTypeToString(n.type),
      n.fromUser?.id ?? '',
      isDebate ? '' : (n.relatedId ?? ''),
      isDebate ? (n.relatedId ?? '') : '',
      n.commentId ?? '',
    ].join('|');
  }

  String _notificationSemanticKeyFromJson(Map<String, dynamic> json) {
    final type = _normalizeNotificationType(json['type'] as String? ?? '');
    final isDebate = type.startsWith('debate_');

    return [
      type,
      (json['from_user_id'] ?? json['actor_id'] ?? '').toString(),
      isDebate ? '' : (json['post_id'] ?? '').toString(),
      isDebate ? (json['discussion_id'] ?? '').toString() : '',
      (json['comment_id'] ?? '').toString(),
    ].join('|');
  }

  bool _isTempNotificationId(String id) => id.startsWith('temp_');
  ...
  Future<void> loadNotifications({bool loadMore = false}) async {
    ...
    try {
      ...
      final fetched = data.map(AppNotification.fromJson).toList();
      ...
      if (loadMore) {
        ...
        // Сохраняем только те временные push-уведомления, которым ещё нет пары из БД
        final fetchedKeys = fetched.map(_notificationSemanticKeyFromApp).toSet();
        final tempNotifications = _notifications.where((n) {
          if (!_isTempNotificationId(n.id)) return false;
          final key = _notificationSemanticKeyFromApp(n);
          return !fetchedKeys.contains(key);
        }).toList();
        _notifications = fetched;
        // Добавляем только действительно отсутствующие временные уведомления обратно
  ...
  void addNotificationFromPush(Map<String, dynamic> data) {
    ...
    try {
      ...
      final semanticKey = _notificationSemanticKey(
        type: type,
      ...
      final alreadyExists = _notifications.any((n) =>
        _notificationSemanticKeyFromApp(n) == semanticKey,
      );
      if (alreadyExists) {
        AppLogger.info('[PUSH-UI] Duplicate push skipped: type=$type', tag: _logTag);
        return;
      }
      ...
      final actorMap = (data['actor'] is Map)
          ? Map<String, dynamic>.from(data['actor'] as Map)
          : (data['from_user_id'] != null
              ? {
                  'id': data['from_user_id'],
                  'full_name': data['actor_name'] ?? 'Пользователь',
                  'username': data['actor_username'] ?? 'user',
      ...
      final notification = AppNotification.fromJson({
        ...
        'type': type,
        'actor': actorMap,
        'title': data['title'] ?? _getNotificationTitle(type),
      ...
      _updateBadgeCount(_unreadNotificationsCount + 1);
      
      notifyListeners();
      
      AppLogger.info('[PUSH-UI] Добавлено уведомление в state: type=$type from=${notification.fromUser?.name}', tag: _logTag);
  ...
  NotificationType _mapNotificationType(String type) {
    switch (_normalizeNotificationType(type)) {
      case 'like':
  ...
  Future<void> markNotificationAsRead(String notificationId) async {
    ...
    // Локально помечаем сразу
    _notifications[index] = notification.copyWith(isRead: true);
    notifyListeners();

    // temp уведомление в БД не отправляем
    if (_isTempNotificationId(notificationId)) {
      AppLogger.info('[NOTIF] temp notification marked read locally only: $notificationId', tag: _logTag);
      return;
    }
  ...
  Future<void> deleteNotification(String notificationId) async {
    ...
    // temp уведомление удаляем только локально
    if (_isTempNotificationId(notificationId)) {
      ...
      AppLogger.info('[NOTIF] temp notification deleted locally only: $notificationId', tag: _logTag);
      await _saveNotifications();
      notifyListeners();
      return;
    }
  ...
  // Загрузить один комментарий по ID (для скролла из уведомлений)
  Future<void> loadCommentById(String postId, String commentId) async {
    if (!Features.useSupabasePosts) return;
    // Проверим, что комментарий уже есть
    final existing = _comments.any((c) => c.storyId == postId && c.id == commentId);
    if (existing) return;
    ...
    try {
      ...
      String? replyToAuthor;
      if (parentId != null) {
        final parent = _comments.where((c) => c.storyId == postId).cast<Comment?>().firstWhere(
          (c) => c?.id == parentId,
          orElse: () => null,
        );
        replyToAuthor = parent?.author.name;
      }
      ...
      final comment = Comment(
        ...
        author: User(
          id: userId,
          username: user?['username'] as String? ?? '',
          name: user?['full_name'] as String? ?? user?['username'] as String? ?? 'Пользователь',
        ...
        replyToAuthor: replyToAuthor,
        replyTargetId: parentId,
        threadRootId: parentId ?? (commentData['id'] as String),
        isPendingSync: false,
        mediaUrl: commentData['media_url'] as String?,
        mediaType: commentData['media_type'] as String?,
        mediaWidth: commentData['media_width'] as int?,
      ...
      _recalculateStoryCommentsCount(postId);
      notifyListeners();
      AppLogger.info('Loaded single comment $commentId for post $postId', tag: _logTag);
    } catch (e) {
      AppLogger.warning('Failed to load comment $commentId', tag: _logTag, error: e);
    }
  }
